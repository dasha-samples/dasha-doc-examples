...
block SlotFilling(slots: Slots, 
    ...
    node slot_asker {
        do {
            ...
            if (digression.slot_filler.droppedSomeSlots == true) {
                #sayText("Ok.");
            }
    ...
    node slot_confirmation {
        ...
        transitions {
            ...
            positive: goto finish_slot_filling on #messageHasSentiment("positive");
            drop_all_slots: goto slot_asker on #messageHasSentiment("negative");
            ...
            repeat_new_fulfilled: goto slot_asker on true priority -100;
        }
        onexit {
            drop_all_slots: do {
            ...
            */
            ...
            for (var key in $slots.keys()) {
                ...
                if (slot.value == (dropSlotsValues[key])?.value) {
                    #log("Dropping slot '" + key + "' by entity...");
                ...
                if (dropSlots[key] == true) {
                    #log("Dropping slot '" + key + "' by intent...");
                    /** uncomment line bellow to bug compiler */
                    // if (slot.value is not null)
                        set digression.slot_filler.droppedSomeSlots = true;
                ...
                if (#messageHasSentiment("negative") and ((digression.mentioned.saved[key])?.value is not null)) {
                    #log("Dropping slot '" + key + "' by negation...");
                    /** uncomment line bellow to bug compiler */
                    // if (slot.value is not null)
                        set digression.slot_filler.droppedSomeSlots = true;
                    set slot = {value: null, values: []};
